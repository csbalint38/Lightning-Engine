...
namespace lightning::graphics::direct3d12::camera {
	namespace {
		...
		void set_up_vector(D3D12Camera camera, const void* const data, u32 size) {
			math::v3 up_vector{ *(math::v3*)data };
			assert(sizeof(up_vector) == size);
			camera.up(up_vector);
		}

		void set_field_of_view(D3D12Camera camera, const void* const data, u32 size) {
		...
		void set_aspect_ratio(D3D12Camera camera, const void* const data, u32 size) {
			assert(camera.projection_type() == graphics::Camera::PERSPECTIVE);
		...
		void set_view_width(D3D12Camera camera, const void* const data, u32 size) {
		...
		void set_view_height(D3D12Camera camera, const void* const data, u32 size) {
			assert(camera.projection_type() == graphics::Camera::ORTOGRAPHIC);
			f32 height{ *(f32*)data };
		...
		void set_near_z(D3D12Camera camera, const void* const data, u32 size) {
			f32 near_z{ *(f32*)data };
			assert(sizeof(near_z) == size);
			camera.near_z(near_z);
		}

		void set_far_z(D3D12Camera camera, const void* const data, u32 size) {
			f32 far_z{ *(f32*)data };
			assert(sizeof(far_z) == size);
			camera.far_z(far_z);
		}

		void get_view(D3D12Camera camera, void* const data, u32 size) {
			math::m4x4* const matrix{ (math::m4x4* const)data };
			assert(sizeof(math::m4x4) == size);
			DirectX::XMStoreFloat4x4(matrix, camera.view());
		}

		void get_projection(D3D12Camera camera, void* const data, u32 size) {
			math::m4x4* const matrix{ (math::m4x4* const)data };
			assert(sizeof(math::m4x4) == size);
			DirectX::XMStoreFloat4x4(matrix, camera.projection());
		}

		void get_inverse_projection(D3D12Camera camera, void* const data, u32 size) {
			math::m4x4* const matrix{ (math::m4x4* const)data };
			assert(sizeof(math::m4x4) == size);
			DirectX::XMStoreFloat4x4(matrix, camera.inverse_projection());
		}

		void get_view_projection(D3D12Camera camera, void* const data, u32 size) {
			math::m4x4* const matrix{ (math::m4x4* const)data };
			assert(sizeof(math::m4x4) == size);
			DirectX::XMStoreFloat4x4(matrix, camera.view_projection());
		}
		void get_inverse_view_projection(D3D12Camera camera, void* const data, u32 size) {
			math::m4x4* const matrix{ (math::m4x4* const)data };
			assert(sizeof(math::m4x4) == size);
			DirectX::XMStoreFloat4x4(matrix, camera.inverse_view_projection());
		}

		void get_up_vector(D3D12Camera camera, void* const data, u32 size) {
			math::v3 *const up_vector{ (math::v3* const )data };
			assert(sizeof(up_vector) == size);
			DirectX::XMStoreFloat3(up_vector, camera.up());
		}

		void get_field_of_view(D3D12Camera camera, void* const data, u32 size) {
			assert(camera.projection_type() == graphics::Camera::PERSPECTIVE);
		...
		void get_aspect_ratio(D3D12Camera camera, void* const data, u32 size) {
			assert(camera.projection_type() == graphics::Camera::PERSPECTIVE);
		...
		void get_view_width(D3D12Camera camera, void* const data, u32 size) {
			assert(camera.projection_type() == graphics::Camera::ORTOGRAPHIC);
		...
		void get_view_height(D3D12Camera camera, void* const data, u32 size) {
			assert(camera.projection_type() == graphics::Camera::ORTOGRAPHIC);
		...
		void get_near_z(D3D12Camera camera, void* const data, u32 size) {
			f32* const near_z{ (f32* const)data };
			assert(sizeof(f32) == size);
			*near_z = camera.near_z();
		}

		void get_far_z(D3D12Camera camera, void* const data, u32 size) {
			f32* const far_z{ (f32* const)data };
			assert(sizeof(f32) == size);
			*far_z = camera.far_z();
		}

		void get_projection_type(D3D12Camera camera, void* const data, u32 size) {
		...
		void get_entity_id(D3D12Camera camera, void* const data, u32 size) {
