...
class VisualStudio {
  ...
  static final getRunningObjectTable = ole32.lookupFunction<Int32 Function(Uint32 reserved, Pointer<Pointer<COMObject>> pprot), int Function(int reserved, Pointer<Pointer<COMObject>> pprot)>('GetRunningObjectTable');
  static final createBindCtx = ole32.lookupFunction<Int32 Function(Uint32 reserved, Pointer<Pointer<COMObject>> ppbc), int Function(int reserved, Pointer<Pointer<COMObject>> ppbc)>('CreateBindCtx');
  ...
  static void openVisualStudio(String solutionPath) {
    ...
    //try {
      if(_vsInstance == nullptr) {
        ...
        if(FAILED(hResult)) throw Exception("GetRunningObjectTable() returned HRESULT: ${hResult.toRadixString(16)}");
        ...
        if(hResult < 0 || bindCtx == nullptr) throw Exception("CreateBindCtx() returned HRESULT: ${hResult.toRadixString(16)}");
        ...
        while(monikerTableInstance.next(1, currentMoniker, nullptr) == S_OK) {
          ...
          if(displayName.contains(_progId)) {
            ...
            if(hResult < 0) throw Exception("RunningObjectTable's GetObject() returned HRESULT: ${hResult.toRadixString(16)}");

            _vsInstance = dte2Pointer.value;

            final solutionNamePtr = calloc<Pointer<Utf16>>();
            final getSolutionName = (dte2Pointer.value.ref.lpVtbl.value+6).cast<Pointer<NativeFunction<HRESULT Function(Pointer, Pointer<Pointer<Utf16>>)>>>().value.asFunction<int Function(Pointer, Pointer<Pointer<Utf16>>)>();
        ...
        if(_vsInstance == nullptr) {
          ...
          if(FAILED(hResult)) throw COMException(hResult);
          hResult = IIDFromString(TEXT('{00000000-0000-0000-C000-000000000046}'), iid);
          if(FAILED(hResult)) throw COMException(hResult);

          hResult = CoCreateInstance(clsid, nullptr, CLSCTX.CLSCTX_LOCAL_SERVER, iid, _vsInstance.cast());
