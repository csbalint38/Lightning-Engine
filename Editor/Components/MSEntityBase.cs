...
abstract public class MSEntityBase : ViewModelBase
{
    ...
    public MSEntityBase(List<Entity> entities)
    {
        ...
        PropertyChanged += (s, e) =>
        {
            if (_enableUpdates && e.PropertyName is not null) UpdateEntities(e.PropertyName);
        };
    }
    ...
    protected virtual bool UpdateEntities(string propertyName)
    {
        switch (propertyName)
        {
            case nameof(IsEnabled):
                SelectedEntities.ForEach(x => x.IsEnabled = IsEnabled ?? false);
                return true;
            case nameof(Name):
                SelectedEntities.ForEach(x => x.Name = Name ?? string.Empty); return true;
        }
    ...
    public T? GetMSComponent<T>() where T : class, IMSComponent =>
