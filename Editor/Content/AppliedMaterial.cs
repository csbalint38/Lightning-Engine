...
public class AppliedMaterial : Asset
{
    ...
    public bool UploadToEngine()
    {
        lock (_lock)
        {
            ...
            if (_packedMaterials.TryGetValue(dataString, out var uploadedAsset))
            {
                ...
                Debug.Assert(((MaterialMetadata)uploadedAsset.Metadata!).PackedData!.SequenceEqual(_packedData));

                Guid = uploadedAsset?.AssetInfo?.Guid ?? Guid.Empty;
    ...
    public void UnloadFromEngine()
    {
        lock (_lock)
        {
            ...
            Debug.Assert(UploadedAsset.GetContentId(UploadedAsset?.AssetInfo?.Guid ?? Guid.Empty) == UploadedAsset?.ContentId);
